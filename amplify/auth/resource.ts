...
/**
 * Cognito access for **auth-stack** functions only (`resourceGroupName: "auth"`).
 * Data-stack Lambdas that need AdminGetUser get IAM via `addToRolePolicy` in
 * `backend.ts` — `allow.resource` on data functions creates auth→data and
 * circular nested-stack deps with storage.
 */
