...
import {
  DEFAULT_CART_CLEANUP_SETTINGS,
  fetchCartCleanupSettings,
  runIdleCartCleanup,
  saveCartCleanupSettings,
  type CartCleanupScope,
  type CartCleanupSettings,
...
export function AdminSettingsPage() {
  ...
  const [settings, setSettings] = useState<CartCleanupSettings>(
    DEFAULT_CART_CLEANUP_SETTINGS,
  );
  ...
  useEffect(() => {
    void (async () => {
      ...
      if (!client) return;
      try {
        setSettings(await fetchCartCleanupSettings(client));
      } catch (err) {
  ...
  async function handleSave(event: FormEvent) {
    ...
    try {
      const client = await requireAdminSession(navigate);
      if (!client) return;
      setSettings(await saveCartCleanupSettings(client, settings));
      setSaved(true);
    } catch (err) {
  ...
  return (
    <div>
      ...
      <form
        ...
        className="mt-stack-lg max-w-2xl space-y-6 border border-outline-variant/20 bg-surface-container-low p-stack-lg iron-bevel"
      >
        <div>
          ...
          <p className="mt-2 text-body-sm text-on-surface-variant">
            ...
            than the threshold. Product active-cart counts are adjusted when rows
            are deleted. A daily job uses the saved settings when enabled;{" "}
        ...
        <label className="block">
          ...
          />
          <span className="mt-1 block text-body-sm text-on-surface-variant">
            Based on last cart sync (<code className="text-on-surface">updatedAt</code>
            ...
            {SCOPE_OPTIONS.map((option) => (
              <label key={option.value} className="flex items-start gap-3">
                <input
                  ...
                  className="mt-1"
                  checked={settings.scope === option.value}
                  onChange={() =>
                    setSettings((prev) => ({ ...prev, scope: option.value }))
                  }
                />
                <span>
                  <span className="font-label-md text-on-surface">
