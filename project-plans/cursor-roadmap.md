...
- **M23c (partial)** — Customer **Gallery** — **production verified** (2026-08-11); hide home Newsletter — **shipped** (repo 2026-08-10); **mobile nav** — **shipped** (repo 2026-08-13); see **§3.10**.
- **M10** — Customer ↔ shop **message inbox** (Etsy-style threads) — **production verified** (2026-08-13): `Conversation`/`Message`, guest + signed-in threads, photos, delete conversations, `/account/messages`, `/admin/messages`, header mail badge, homepage + Contact CTAs
...
| **Mobile nav** | **Shipped** (repo 2026-08-13) — header menu button + panel: Shop / Gallery / Vault / About / Contact + search |
...
**M23** — Storefront trust & legitimacy (**M23a** + **M23b** + **M23c** Gallery + mobile nav done; Newsletter hide shipped; **M23d–f** remain).
...
- **M23c** — Hide unfinished Newsletter; enable **customer Gallery** — Gallery **production verified** (2026-08-11); Newsletter hide **done**; **mobile nav shipped** (repo 2026-08-13). Newsletter **implementation deferred to M13b**.
- **M23d** — FAQ page
...
- **M10** — Customer ↔ shop **message inbox** (Etsy-style threads; not live tech-support chat) — **production verified** 2026-08-13
- **M23d–f** — FAQ / contact hours / Settings social links / reviews seed
...
### M6e — Guest identity parity

**Status:** **Production verified** 2026-08-09 (foundation 2026-08-06 · cart 2026-08-07 · favorites / prints / guest inbox / admin activity 2026-08-09).

**Goal met:** Shoppers can **browse, favorite, cart, checkout, and submit custom print requests without creating an account**. Cognito remains an optional upgrade for order history, promo redemption, cross-device sync, and the signed-in account inbox. Shared foundation: stable **`guestId`** (Function URL cookie + HMAC `guestToken` + localStorage mirror) that owns server rows parallel to `userId`, with **idempotent merge on sign-in/register**.

**Shipped surface**
- **Cart** — `GuestCartSnapshot`; `syncCartSnapshot` + `loadGuestCartSnapshot` hydrate; counts include guests; merge deletes guest cart.
- **Favorites** — `GuestFavorite`; PDP save + `/account/favorites` signed-out; merge → `Favorite` (+ deferred grant).
- **Prints** — `PrintRequest.guestId` + contact `email`; guest upload/submit/list/pay quote; merge attaches to `userId`.
...
2. All guest-mutating Lambdas: verify HMAC `guestToken` for `guestId` (AppSync cannot receive Function URL HttpOnly cookies).
3. **Sign-in / register merge** (`mergeGuestIdentity`): cart → favorites → print requests → notifications → recompute product counts → deferred grants under `userId`. Clears local guest credentials on success.
...
**Verified (2026-08-07 + hydrate 2026-08-08):** **Option B** — `GuestCartSnapshot` model (PK `guestId`) parallel to `CartSnapshot` (PK `userId`).

- **`syncCartSnapshot`** — guest HMAC path; empty sync gated until hydrate.
- **`loadGuestCartSnapshot`** — restore UI when localStorage cart empty.
...
**Verified (2026-08-09):** `GuestFavorite`; `toggleProductFavorite` + `getGuestFavorites`; PDP + `/account/favorites` signed-out; merge + deferred favorite grant.

---

#### C — Custom print requests (M21c)

**Verified (2026-08-09):** Guest submit/list/pay quote; admin quote → **GuestNotification**; account menu links for guests; merge attaches open requests to the Cognito user.

**Notifications:** `GuestNotification` inbox (not Cognito `Notification.userId`). SES email remains optional.
...
- Print request queue shows guest badge / email when `guestId` set.
- Customer carts & favorites lists active guests (`Guest · …`); rows removed after merge.
...
#### Acceptance — met (2026-08-09)

Cart / favorites / prints / guest inbox / merge verified in production testing. Optional regressions: overlapping cart merge quantities; tampered `guestToken`; cart hydrate after clearing only `emperium-cart`.

**Cursor rules:**
- Reuse cart/favorite count helpers; do not fork second count implementations.
...
- Wrong/missing `guestToken` cannot cancel another guest’s order.
- After ship, cancel hidden; returns still account-only unless a later milestone expands them.
- `/shipping-returns` text matches live behavior.

**Out of scope (M16e):** Guest returns portal; guest thank-you promo grants; email cancel links.
...
**Status:** **Production verified** (2026-08-13). Guest + signed-in threads, photo attachments, conversation delete (customer + admin).
...
**Auth:**
- Customer: owner create/read/update/delete on conversations where `userId = sub`.
- Guest: HMAC `guestId` + `guestToken` via `guest-messages` Lambda (same pattern as guest orders/notifications); merge into account on sign-in; delete via Lambda.
- Admin: read/update/delete; unread thread count/badge.

**Frontend:**
- Customer: `/account/messages` — thread list, open thread, reply, start new conversation, **delete conversation** (Account / Contact entry; optional “Ask about this order” on order detail).
- Admin: `/admin/messages` — thread list, filter, reply, **delete conversation**; deep-link from order detail when `orderId` set.
...
9. ~~No brand **social / Etsy** links in footer/header~~ — **planned M23e**: Admin → Settings manages links (network + URL); storefront shows matching icons (Etsy reviews URL alone is not enough)
...
| **M23c** | Hide disabled Newsletter; enable customer Gallery (`GalleryEntry` + admin + `/gallery` + `gallery/*`); **mobile nav** for Shop / About / Contact | High | **Done** — Gallery **production verified** (2026-08-11); Newsletter **hidden** (implement later in **M13b**); mobile nav **shipped** (repo 2026-08-13) |
...
##### M23e — Brand social links (Admin → Settings)

**Goal:** Admins add the store’s social profiles in **Admin → Settings** (same panel as cart cleanup). Each row: choose the **network** from a dropdown + paste the **profile URL**. The public shop shows the correct **icon** for that network wherever socials are displayed (footer first; optional header/home later).

**Admin UX**
- Settings section: **Social media** — list of links; Add / Edit / Remove / reorder (sortOrder).
- Fields per link: `network` (enum dropdown), `url` (https), optional `label` (defaults to network name), `active`.
- Supported networks (v1 dropdown): **Etsy**, **Instagram**, **Facebook**, **X (Twitter)**, **YouTube**, **TikTok**, **Pinterest**, **Discord** (extend enum later if needed).
- Validate URL is http(s); optionally warn if hostname doesn’t match the chosen network.

**Data**
- Prefer JSON on `CatalogSettings` (e.g. `socialLinks: a.json()`) **or** a small `StoreSocialLink` model — Settings singleton is enough for v1.
- Do **not** hardcode brand social URLs only in `src/lib/config.ts` once this ships (Etsy **reviews** deep-link may remain separate).

**Storefront**
- Footer (and any dedicated “find us” strip) renders active links with network icons (`Icon` / brand SVG set — one icon per enum value).
- Icons are decorative beside accessible link text or `aria-label` with the network name.
- Empty list → render nothing (no broken placeholders).

**Acceptance**
- Admin saves Instagram + Etsy URLs → footer shows both icons linking correctly after refresh.
- Removing or deactivating a link hides it on the shop.
- Wrong/empty URL fails validation in Settings; no blank icon row on the storefront.
...
- `Header.tsx` — ~~stop linking disabled Gallery~~ (**done** → `/gallery`); ~~mobile navigation~~ (**done** 2026-08-13)
...
- `src/lib/config.ts` — stop being the only source for brand socials once M23e ships (Etsy reviews URL may stay)
...
- ~~Mobile can reach Shop, About, Contact from header.~~ **Met** (repo 2026-08-13).
- No disabled Newsletter/Gallery controls that look like broken features. ~~Newsletter hidden; Gallery production verified; mobile nav shipped.~~
...
3. **Auth:** Guest submit + pay quote is **M6e** (cookie `guestId` + contact email). Cognito optional for account history.
