...
- **Kill switch / paths:** Admin → Settings → **Email** — master enable plus per-path toggles (new-order support, order paid/shipped, messages, print quote/decline). Stored on `CatalogSettings`; unset = on.
- **Messages (M10):** Guest compose email is optional; if set (or Cognito email for accounts), shop replies trigger a Resend alert when the shop-message path is on.
